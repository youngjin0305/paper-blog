...
등록 스크립트의 일간 작업은 `--resume-draft`를 포함합니다. 큐에 선택 가능한 논문의 초안이 있으면 재검증 후 재사용하고, 없으면 정상 생성합니다. `failed` 논문은 기존 규칙대로 `add`로 다시 고정해야 합니다. 쿼터 초과 시에는 큐 상태를 유지하므로 다음 예약 실행에서 재개할 수 있습니다.

agy 내부 재시도에서 쿼터 오류가 stdout/stderr에 나타나지 않을 수 있으므로, `--help`로 확인한 `--log-file`을 호출별 작업 폴더에 지정하고 로그도 감시합니다. `429`/`RESOURCE_EXHAUSTED`를 감지하면 출력 파일이 있어도 해당 호출과 하위 프로세스를 즉시 종료합니다. 원시 CLI 로그는 저장소에 게시하지 않습니다.
